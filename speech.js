(() => {
  ...
  function systemSpeak(text, rate = 0.92) {
    const synth = window.speechSynthesis;
    if (!synth || typeof SpeechSynthesisUtterance === "undefined") {
      notify("此浏览器不支持系统语音。可以为固定口令录制或上传真人语音。");
      return false;
    }
    ...
    utterance.onerror = event => {
      if (!["canceled", "interrupted"].includes(event.error)) notify("系统语音播放失败，请换一个中文音色或使用真人录音。");
    };
    try {
  ...
  async function open(text = null) {
    ...
    if (!dialog) {
      ...
      dialog.innerHTML = `
        ...
        <p class="help">系统语音会优先选择设备里的高质量中文音色，并用较慢、较低音调的“自然播报”参数。若想完全去掉机器感，最稳妥的方法是给常用固定口令绑定真人录音。</p>
        ...
        <div class="row"><button class="btn primary" id="voicePreview" type="button">试听</button><button class="btn ghost" id="voiceStop" type="button">停止播报</button></div>
        <hr class="sep">
  ...
  window.JudgeSpeech = { speak, stop, open, naturalize };
})();
